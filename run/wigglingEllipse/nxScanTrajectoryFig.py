...
figname="wigglingEllipseNxScanTrajectories.pdf"

scanName='maxCoScan_newOde'
maxCo='0.2'
...
tquiv = np.arange(start=t2[0], stop=t2[-1], step=dtquiver)
...
for i in range(len(nxlist)):
    caseDir='maxCo'+maxCo+'nx'+str(nxlist[i])
    os.chdir(caseDir)
    ...
    line, = plt.plot(x1, y1, linewidth=2, zorder=-i,label=str(nxlist[i]))
    col = line.get_color() 
